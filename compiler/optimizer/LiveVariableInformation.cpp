...
/*
 * Given an OSRPoint, set any symbols that are alive in the corresponding bci in interpreter.
 */
void 
 ...
 * 
 * @param osrMethodData
 * @param autosOrPPSListArray List of all the autos or pending push symbols in a given method
...
TR_OSRLiveVariableInformation::buildLiveSymbolsBitVector(TR_OSRMethodData *osrMethodData, int32_t byteCodeIndex, TR_BitVector *liveLocals)
   ...
   while (liveSymRefIt.hasMoreElements())
      ...
      traceMsg(comp(), "buildLiveSymbolsBitVector check if symRef #%d is alive\n", symRefNumber);
...
/*
 * \brief Calculate live symbols at a given bci in the current call chain
 */
TR_BitVector *
TR_OSRLiveVariableInformation::getLiveSymbolsInInterpreter(TR_ByteCodeInfo &byteCodeInfo)
   ...
   traceMsg(comp(), "getLiveSymbolsInInterpreter for %d:%d methodSymbol %p\n", callerIndex, byteCodeIndex, methodSymbol);
   TR_OSRMethodData *osrMethodData = comp()->getOSRCompilationData()->getOSRMethodDataArray()[callerIndex+1];
   TR_BitVector *liveLocals = new (comp()->trHeapMemory()) TR_BitVector(0, comp()->trMemory(), heapAlloc);
